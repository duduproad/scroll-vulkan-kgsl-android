...
// wlroots-kgsl: set up a render buffer that targets a host (shm) wlr_buffer.
...
struct wlr_renderer *wlr_vk_renderer_create_with_drm_fd(int drm_fd) {
	...
	if (!phdev) {
		wlr_log(WLR_INFO, "Could not match drm and vulkan device, falling back to first physical device");
		...
		if (!phdev) {
			wlr_log(WLR_ERROR, "Failed to find any Vulkan physical devices");
			goto error;
		}
	}
